...
class User_Received_Message_Unthrottled_Event extends \Voxel\Events\Base_Event {
	...
	public function get_key(): string {
		return 'messages/user:received_message_unthrottled';
	}

	public function get_label(): string {
		return 'Messages: User received new message (unthrottled)';
	}

	public function get_description(): ?string {
		return 'This event is triggered for <strong>every</strong> received message. Enabling email or in-app notifications is not recommended for this event. You can use the throttled version instead for that purpose.';
	}
	...
	public function set_mock_props() {
		...
		$this->message = \Voxel\Modules\Direct_Messages\Message::mock();
	}

	public function dynamic_tags(): array {
		return [
			'user' => \Voxel\Dynamic_Data\Group::User( $this->user ),
			'message' => \Voxel\Dynamic_Data\Group::Message( $this->message ),
		];
	}
}
