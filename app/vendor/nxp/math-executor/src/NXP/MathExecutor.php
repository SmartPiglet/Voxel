...
class MathExecutor
{
     ...
     */
    public function execute(string $expression, bool $cache = \true)
    {
        $cacheKey = $expression;
     ...
     * @throws \Exception
     */
    public function setVars(array $variables, bool $clear = \true): self
    {
        if ($clear) {
            $this->removeVars();
        }
    ...
    public function useBCMath(int $scale = 2): self
    {
        ...
        $this->addOperator(new Operator('^', \true, 220, static fn($a, $b) => \bcpow("{$a}", "{$b}")));
    ...
    protected function defaultFunctions(): array
    {
        ...
        }, 'intdiv' => static fn($arg1, $arg2) => \intdiv($arg1, $arg2), 'ln' => static fn($arg1, $arg2 = \M_E) => \log($arg1, $arg2), 'lg' => static fn($arg) => \log10($arg), 'log' => static fn($arg1, $arg2 = \M_E) => \log($arg1, $arg2), 'log10' => static fn($arg) => \log10($arg), 'log1p' => static fn($arg) => \log1p($arg), 'max' => static function ($arg1, ...$args) {
    ...
    {
        return ['pi' => 3.14159265359, 'e' => 2.71828182846];
    }
    /**
