...
use Voxel\Vendor\Stripe\Util\Util;
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    public function __construct($config = [])
    {
        ...
        $this->config = $config;
        $this->defaultOpts = \Voxel\Vendor\Stripe\Util\RequestOptions::parse(['stripe_account' => $config['stripe_account'], 'stripe_context' => $config['stripe_context'], 'stripe_version' => $config['stripe_version'], 'max_network_retries' => $config['max_network_retries']]);
    }
    /**
    ...
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        } else if (!$obj instanceof V2\Collection) {
            $received_class = \get_class($obj);
    ...
    {
        return Util::convertToStripeObject(\json_decode($json, \true), [], $apiMode);
    }
    /**
    ...
    public function parseThinEvent($payload, $sigHeader, $secret, $tolerance = Webhook::DEFAULT_TOLERANCE)
    {
        ...
        try {
            return Util::json_decode_thin_event_object($eventData, 'Voxel\Vendor\Stripe\ThinEvent');
