 ...
 */
namespace Voxel\Vendor\Symfony\Component\OptionsResolver;

use Voxel\Vendor\Symfony\Component\OptionsResolver\Exception\AccessException;
use Voxel\Vendor\Symfony\Component\OptionsResolver\Exception\InvalidArgumentException;
use Voxel\Vendor\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException;
use Voxel\Vendor\Symfony\Component\OptionsResolver\Exception\MissingOptionsException;
use Voxel\Vendor\Symfony\Component\OptionsResolver\Exception\NoSuchOptionException;
use Voxel\Vendor\Symfony\Component\OptionsResolver\Exception\OptionDefinitionException;
use Voxel\Vendor\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException;
/**
...
class OptionsResolver implements Options
{
    ...
    public function setDefault(string $option, mixed $value): static
    {
        ...
        if ($value instanceof \Closure) {
            ...
            if (isset($params[0]) && ($type = $params[0]->getType()) instanceof \ReflectionNamedType && self::class === $type->getName() && (!isset($params[1]) || ($type = $params[1]->getType()) instanceof \ReflectionNamedType && Options::class === $type->getName())) {
                ...
                $this->deprecatedNestedOptions[$option] = \true;
                // Store closure for later evaluation
    ...
    public function setDeprecated(string $option, string $package, string $version, string|\Closure $message = 'The option "%name%" is deprecated.'): static
    {
        ...
        $this->deprecated[$option] = ['package' => $package, 'version' => $version, 'message' => $message];
        // Make sure the option is processed
        unset($this->resolved[$option]);
        return $this;
    }
     ...
     *                                   lazy options and/or normalizers
     */
    public function offsetGet(mixed $option, bool $triggerDeprecation = \true): mixed
    {
        if (!$this->locked) {
        ...
        if (isset($this->allowedValues[$option])) {
            ...
            if (!$success) {
                $message = \sprintf('The option "%s" with value %s is invalid.', $this->formatOptions([$option]), $this->formatValue($value));
    ...
    private function verifyTypes(string $type, mixed $value, ?array &$invalidTypes = null, int $level = 0): bool
    {
        ...
        if (\count($allowedTypes) > 1) {
            ...
            if (\is_array($invalidTypes) && (!$invalidTypes || $level > 0)) {
                $invalidTypes[get_debug_type($value)] = \true;
            }
            return \false;
        }
        $type = $allowedTypes[0];
     ...
     * @return $this
     */
    public function setIgnoreUndefined(bool $ignore = \true): static
    {
