 ...
 */
namespace Voxel\Vendor\Symfony\Component\PropertyInfo\Extractor;

use Voxel\Vendor\Symfony\Component\PropertyInfo\PropertyAccessExtractorInterface;
use Voxel\Vendor\Symfony\Component\PropertyInfo\PropertyInitializableExtractorInterface;
use Voxel\Vendor\Symfony\Component\PropertyInfo\PropertyListExtractorInterface;
use Voxel\Vendor\Symfony\Component\PropertyInfo\PropertyReadInfo;
use Voxel\Vendor\Symfony\Component\PropertyInfo\PropertyReadInfoExtractorInterface;
use Voxel\Vendor\Symfony\Component\PropertyInfo\PropertyTypeExtractorInterface;
use Voxel\Vendor\Symfony\Component\PropertyInfo\PropertyWriteInfo;
use Voxel\Vendor\Symfony\Component\PropertyInfo\PropertyWriteInfoExtractorInterface;
use Voxel\Vendor\Symfony\Component\PropertyInfo\Type as LegacyType;
use Voxel\Vendor\Symfony\Component\String\Inflector\EnglishInflector;
use Voxel\Vendor\Symfony\Component\String\Inflector\InflectorInterface;
...
use Voxel\Vendor\Symfony\Component\TypeInfo\TypeContext\TypeContextFactory;
use Voxel\Vendor\Symfony\Component\TypeInfo\TypeIdentifier;
use Voxel\Vendor\Symfony\Component\TypeInfo\TypeResolver\ReflectionParameterTypeResolver;
use Voxel\Vendor\Symfony\Component\TypeInfo\TypeResolver\ReflectionPropertyTypeResolver;
use Voxel\Vendor\Symfony\Component\TypeInfo\TypeResolver\ReflectionReturnTypeResolver;
use Voxel\Vendor\Symfony\Component\TypeInfo\TypeResolver\ReflectionTypeResolver;
use Voxel\Vendor\Symfony\Component\TypeInfo\TypeResolver\TypeResolver;
use Voxel\Vendor\Symfony\Component\TypeInfo\TypeResolver\TypeResolverInterface;
/**
...
class ReflectionExtractor implements PropertyListExtractorInterface, PropertyTypeExtractorInterface, PropertyAccessExtractorInterface, PropertyInitializableExtractorInterface, PropertyReadInfoExtractorInterface, PropertyWriteInfoExtractorInterface, ConstructorArgumentTypeExtractorInterface
{
    ...
    private const MAP_TYPES = ['integer' => TypeIdentifier::INT->value, 'boolean' => TypeIdentifier::BOOL->value, 'double' => TypeIdentifier::FLOAT->value];
     ...
     */
    public function __construct(?array $mutatorPrefixes = null, ?array $accessorPrefixes = null, ?array $arrayMutatorPrefixes = null, private bool $enableConstructorExtraction = \true, int $accessFlags = self::ALLOW_PUBLIC, ?InflectorInterface $inflector = null, private int $magicMethodsFlags = self::ALLOW_MAGIC_GET | self::ALLOW_MAGIC_SET)
    {
    ...
    public function getType(string $class, string $property, array $context = []): ?Type
    {
        ...
        if ($mutatorReflection) {
            try {
                ...
                if (!$type instanceof CollectionType && \in_array($prefix, $this->arrayMutatorPrefixes, \true)) {
    ...
    private function extractFromMutator(string $class, string $property): ?array
    {
        ...
        if (1 === \count($type) && \in_array($prefix, $this->arrayMutatorPrefixes, \true)) {
            $type = [new LegacyType(LegacyType::BUILTIN_TYPE_ARRAY, $this->isNullableProperty($class, $property), null, \true, new LegacyType(LegacyType::BUILTIN_TYPE_INT), $type[0])];
        }
        return $type;
    }
    /**
    ...
    private function extractFromReflectionType(\ReflectionType $reflectionType, \ReflectionClass $declaringClass): array
    {
        ...
        foreach ($reflectionType instanceof \ReflectionUnionType || $reflectionType instanceof \ReflectionIntersectionType ? $reflectionType->getTypes() : [$reflectionType] as $type) {
            ...
            if (LegacyType::BUILTIN_TYPE_ARRAY === $phpTypeOrClass) {
                $types[] = new LegacyType(LegacyType::BUILTIN_TYPE_ARRAY, $nullable, null, \true);
    ...
    private function getPropertyName(string $methodName, array $reflectionProperties): ?string
    {
        ...
        if ('' !== $pattern && preg_match('/^(' . $pattern . ')(.+)$/i', $methodName, $matches)) {
            if (!\in_array($matches[1], $this->arrayMutatorPrefixes, \true)) {
                return $matches[2];
            }
            foreach ($reflectionProperties as $reflectionProperty) {
