 ...
 */
namespace Voxel\Vendor\Symfony\Component\PropertyInfo\Util;

use Voxel\Vendor\phpDocumentor\Reflection\PseudoType;
use Voxel\Vendor\phpDocumentor\Reflection\PseudoTypes\ConstExpression;
use Voxel\Vendor\phpDocumentor\Reflection\PseudoTypes\List_;
use Voxel\Vendor\phpDocumentor\Reflection\Type as DocType;
use Voxel\Vendor\phpDocumentor\Reflection\Types\Array_;
use Voxel\Vendor\phpDocumentor\Reflection\Types\Collection;
use Voxel\Vendor\phpDocumentor\Reflection\Types\Compound;
use Voxel\Vendor\phpDocumentor\Reflection\Types\Integer;
use Voxel\Vendor\phpDocumentor\Reflection\Types\Null_;
use Voxel\Vendor\phpDocumentor\Reflection\Types\Nullable;
use Voxel\Vendor\phpDocumentor\Reflection\Types\String_;
...
final class PhpDocTypeHelper
{
    ...
    private function createLegacyType(DocType $type, bool $nullable): ?LegacyType
    {
        ...
        if ($type instanceof Collection) {
            ...
            if ($fqsen && 'list' === $fqsen->getName() && !class_exists(List_::class, \false) && !class_exists((string) $fqsen)) {
                // Workaround for phpdocumentor/type-resolver < 1.6
                return new LegacyType(LegacyType::BUILTIN_TYPE_ARRAY, $nullable, null, \true, new LegacyType(LegacyType::BUILTIN_TYPE_INT), $this->getTypes($type->getValueType()));
            }
        ...
        if (str_ends_with($docType, '[]') && $type instanceof Array_) {
            ...
            return new LegacyType(LegacyType::BUILTIN_TYPE_ARRAY, $nullable, null, \true, $collectionKeyTypes, $collectionValueTypes);
        }
        ...
        if ('array' === $docType) {
            return new LegacyType(LegacyType::BUILTIN_TYPE_ARRAY, $nullable, null, \true, null, null);
        }
    ...
    {
        if (\in_array($docType, TypeIdentifier::values(), \true)) {
            return [$docType, null];
        }
