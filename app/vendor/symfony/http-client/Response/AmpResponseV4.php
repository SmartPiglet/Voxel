...
use Voxel\Vendor\Amp\ByteStream\StreamException;
use Voxel\Vendor\Amp\CancellationTokenSource;
use Voxel\Vendor\Amp\Coroutine;
use Voxel\Vendor\Amp\Deferred;
use Voxel\Vendor\Amp\Http\Client\HttpException;
use Voxel\Vendor\Amp\Http\Client\Request;
use Voxel\Vendor\Amp\Http\Client\Response;
use Voxel\Vendor\Amp\Loop;
use Voxel\Vendor\Amp\Promise;
use Voxel\Vendor\Amp\Success;
use Voxel\Vendor\Psr\Log\LoggerInterface;
use Voxel\Vendor\Symfony\Component\HttpClient\Chunk\FirstChunk;
use Voxel\Vendor\Symfony\Component\HttpClient\Chunk\InformationalChunk;
use Voxel\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException;
use Voxel\Vendor\Symfony\Component\HttpClient\Exception\TransportException;
use Voxel\Vendor\Symfony\Component\HttpClient\HttpClientTrait;
use Voxel\Vendor\Symfony\Component\HttpClient\Internal\AmpBodyV4;
use Voxel\Vendor\Symfony\Component\HttpClient\Internal\AmpClientStateV4;
use Voxel\Vendor\Symfony\Component\HttpClient\Internal\Canary;
use Voxel\Vendor\Symfony\Component\HttpClient\Internal\ClientState;
...
final class AmpResponseV4 implements ResponseInterface, StreamableInterface
{
     ...
     * @internal
     */
    public function __construct(private AmpClientStateV4 $multi, Request $request, array $options, ?LoggerInterface $logger)
    {
    ...
    {
        $timeout += hrtime(\true) / 1000000000.0;
        self::$delay = Loop::defer(static function () use ($timeout) {
            if (0 < $timeout -= hrtime(\true) / 1000000000.0) {
    ...
    private static function followRedirects(Request $originRequest, AmpClientStateV4 $multi, array &$info, array &$headers, CancellationTokenSource $canceller, array $options, \Closure $onProgress, &$handle, ?LoggerInterface $logger, Promise &$pause): \Generator
    {
        ...
        while (\true) {
            ...
            if (303 === $status || \in_array($status, [301, 302], \true) && 'POST' === $response->getRequest()->getMethod()) {
    ...
    private static function getPushedResponse(Request $request, AmpClientStateV4 $multi, array &$info, array &$headers, array $options, ?LoggerInterface $logger): \Generator
    {
        ...
        foreach ($multi->pushedResponses[$authority] ?? [] as $i => [$pushedUrl, $pushDeferred, $pushedRequest, $pushedResponse, $parentOptions]) {
            ...
            $info += ['connect_time' => 0.0, 'pretransfer_time' => 0.0, 'starttransfer_time' => 0.0, 'total_time' => 0.0, 'namelookup_time' => 0.0, 'primary_ip' => '', 'primary_port' => 0, 'start_time' => microtime(\true)];
