...
use Voxel\Vendor\Amp\DeferredCancellation;
use Voxel\Vendor\Amp\DeferredFuture;
use Voxel\Vendor\Amp\Future;
...
use Voxel\Vendor\Revolt\EventLoop;
...
use Voxel\Vendor\Symfony\Component\HttpClient\Internal\AmpBodyV5;
use Voxel\Vendor\Symfony\Component\HttpClient\Internal\AmpClientStateV5;
...
use function Voxel\Vendor\Amp\delay;
use function Voxel\Vendor\Amp\Future\awaitFirst;
/**
...
final class AmpResponseV5 implements ResponseInterface, StreamableInterface
{
     ...
     * @internal
     */
    public function __construct(private AmpClientStateV5 $multi, Request $request, array $options, ?LoggerInterface $logger)
    {
    ...
    private static function generateResponse(Request $request, AmpClientStateV5 $multi, string $id, array &$info, array &$headers, DeferredCancellation $canceller, array &$options, \Closure $onProgress, &$handle, ?LoggerInterface $logger, float &$pause): void
    {
        ...
        try {
            ...
            while (\true) {
                ...
                $multi->openHandles[$id] = new DeferredFuture();
                if (0 < $pause) {
                    delay($pause, \true, $canceller->getCancellation());
                }
