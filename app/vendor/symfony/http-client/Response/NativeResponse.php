...
use Voxel\Vendor\Symfony\Component\HttpClient\Internal\NativeClientState;
...
final class NativeResponse implements ResponseInterface, StreamableInterface
{
     ...
     */
    public function __construct(private NativeClientState $multi, private $context, private string $url, array $options, array &$info, callable $resolver, ?callable $onProgress, ?LoggerInterface $logger)
    {
        ...
        $info['pause_handler'] = static function (float $duration) use (&$pauseExpiry) {
            $pauseExpiry = 0 < $duration ? hrtime(\true) / 1000000000.0 + $duration : 0;
        };
    ...
    private static function perform(ClientState $multi, ?array $responses = null): void
    {
        foreach ($multi->openHandles as $i => [$pauseExpiry, $h, $buffer, $onProgress]) {
            if ($pauseExpiry) {
                if (hrtime(\true) / 1000000000.0 < $pauseExpiry) {
                    continue;
                }
        ...
        foreach ($responses as $i => $response) {
            ...
            if ($response->pauseExpiry && hrtime(\true) / 1000000000.0 < $response->pauseExpiry) {
    ...
    private static function select(ClientState $multi, float $timeout): int
    {
        ...
        foreach ($multi->openHandles as [$pauseExpiry, $h]) {
            ...
            if ($pauseExpiry && ($now ??= hrtime(\true) / 1000000000.0) < $pauseExpiry) {
