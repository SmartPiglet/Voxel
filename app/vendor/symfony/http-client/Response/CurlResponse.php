...
use Voxel\Vendor\Symfony\Component\HttpClient\Internal\CurlClientState;
...
final class CurlResponse implements ResponseInterface, StreamableInterface
{
     ...
     * @internal
     */
    public function __construct(private CurlClientState $multi, \CurlHandle|string $ch, ?array $options = null, ?LoggerInterface $logger = null, string $method = 'GET', ?callable $resolveRedirect = null, ?int $curlVersion = null, ?string $originalUrl = null)
    {
        ...
        $this->info['pause_handler'] = static function (float $duration) use ($ch, $multi, $execCounter) {
            if (0 < $duration) {
                ...
                $multi->pauseExpiries[(int) $ch] = $duration += hrtime(\true) / 1000000000.0;
    ...
    private static function perform(ClientState $multi, ?array $responses = null): void
    {
        ...
        try {
            ...
            while ($info = curl_multi_info_read($multi->handle)) {
                ...
                $multi->handlesActivity[$id][] = null;
                $multi->handlesActivity[$id][] = \in_array($result, [\CURLE_OK, \CURLE_TOO_MANY_REDIRECTS], \true) || '_0' === $waitFor || curl_getinfo($ch, \CURLINFO_SIZE_DOWNLOAD) === curl_getinfo($ch, \CURLINFO_CONTENT_LENGTH_DOWNLOAD) || 'C' === $waitFor[0] && 'OpenSSL SSL_read: SSL_ERROR_SYSCALL, errno 0' === curl_error($ch) && -1.0 === curl_getinfo($ch, \CURLINFO_CONTENT_LENGTH_DOWNLOAD) && \in_array('close', array_map('strtolower', $responses[$id]->headers['connection'] ?? []), \true) ? null : new TransportException(ucfirst(curl_error($ch) ?: curl_strerror($result)) . \sprintf(' for "%s".', curl_getinfo($ch, \CURLINFO_EFFECTIVE_URL)));
            }
        } finally {
    ...
    private static function select(ClientState $multi, float $timeout): int
    {
        ...
        if ($multi->pauseExpiries && 0 < $timeout -= hrtime(\true) / 1000000000.0 - $now) {
