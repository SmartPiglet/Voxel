...
use Voxel\Vendor\Symfony\Component\HttpClient\Chunk\LastChunk;
...
trait TransportResponseTrait
{
    ...
    public static function stream(iterable $responses, ?float $timeout = null): \Generator
    {
        ...
        $lastActivity = hrtime(\true) / 1000000000.0;
        $elapsedTimeout = 0;
        ...
        while (\true) {
            ...
            if (-1 === self::select($multi, min($timeoutMin, $timeoutMax - $elapsedTimeout))) {
                usleep((int) min(500, 1000000.0 * $timeoutMin));
            }
            $elapsedTimeout = hrtime(\true) / 1000000000.0 - $lastActivity;
        }
    }
}
