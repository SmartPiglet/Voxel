 ...
 */
namespace Voxel\Vendor\Symfony\Component\HttpClient\DependencyInjection;
...
use Voxel\Vendor\Symfony\Component\DependencyInjection\ContainerInterface;
use Voxel\Vendor\Symfony\Component\DependencyInjection\Reference;
...
final class HttpClientPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        ...
        foreach ($container->findTaggedServiceIds('http_client.client') as $id => $tags) {
            $container->register('.debug.' . $id, TraceableHttpClient::class)->setArguments([new Reference('.debug.' . $id . '.inner'), new Reference('debug.stopwatch', ContainerInterface::IGNORE_ON_INVALID_REFERENCE), new Reference('profiler.is_disabled_state_checker', ContainerInterface::IGNORE_ON_INVALID_REFERENCE)])->addTag('kernel.reset', ['method' => 'reset'])->setDecoratedService($id, null, 5);
