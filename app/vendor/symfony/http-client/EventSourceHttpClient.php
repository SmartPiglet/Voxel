...
use Voxel\Vendor\Symfony\Component\HttpClient\Chunk\DataChunk;
use Voxel\Vendor\Symfony\Component\HttpClient\Chunk\ServerSentEvent;
use Voxel\Vendor\Symfony\Component\HttpClient\Exception\EventSourceException;
use Voxel\Vendor\Symfony\Component\HttpClient\Response\AsyncContext;
use Voxel\Vendor\Symfony\Component\HttpClient\Response\AsyncResponse;
...
final class EventSourceHttpClient implements HttpClientInterface, ResetInterface
{
    ...
    {
        return $this->request($method, $url, self::mergeDefaultOptions($options, ['buffer' => \false, 'headers' => ['Accept' => 'text/event-stream', 'Cache-Control' => 'no-cache']], \true));
    }
    public function request(string $method, string $url, array $options = []): ResponseInterface
    {
        ...
        return new AsyncResponse($this->client, $method, $url, $options, static function (ChunkInterface $chunk, AsyncContext $context) use ($state, $method, $url, $options) {
            ...
            try {
                ...
                $state->lastError = $lastError ?? hrtime(\true) / 1000000000.0;
                if (null === $state->buffer || $isTimeout && hrtime(\true) / 1000000000.0 - $state->lastError < $state->reconnectionTime) {
                    yield $chunk;
                } else {
                    $options['headers']['Last-Event-ID'] = $state->lastEventId;
                    $state->buffer = '';
                    $state->lastError = hrtime(\true) / 1000000000.0;
