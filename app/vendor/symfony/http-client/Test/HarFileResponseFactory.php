 ...
 */
namespace Voxel\Vendor\Symfony\Component\HttpClient\Test;
...
class HarFileResponseFactory
{
    ...
    public function __invoke(string $method, string $url, array $options): ResponseInterface
    {
        ...
        $json = json_decode(json: file_get_contents($this->archiveFile), associative: \true, flags: \JSON_THROW_ON_ERROR);
        foreach ($json['log']['entries'] as $entry) {
            ...
            $info = ['http_code' => $response['status'], 'http_method' => $entryMethod, 'response_headers' => [], 'start_time' => strtotime($startedDateTime), 'url' => $entryUrl];
