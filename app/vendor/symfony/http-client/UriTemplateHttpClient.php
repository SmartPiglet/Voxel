...
class UriTemplateHttpClient implements HttpClientInterface, ResetInterface
{
    ...
    {
        if (class_exists(\Voxel\Vendor\GuzzleHttp\UriTemplate\UriTemplate::class)) {
            return \Voxel\Vendor\GuzzleHttp\UriTemplate\UriTemplate::expand(...);
        }
        if (class_exists(\Voxel\Vendor\League\Uri\UriTemplate::class)) {
            return static fn(string $url, array $vars): string => (new \Voxel\Vendor\League\Uri\UriTemplate($url))->expand($vars);
        }
        if (class_exists(\Voxel\Vendor\Rize\UriTemplate::class)) {
            return (new \Voxel\Vendor\Rize\UriTemplate())->expand(...);
        }
