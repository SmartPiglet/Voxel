...
use Voxel\Vendor\Psr\Http\Message\RequestInterface as Psr7RequestInterface;
...
use Voxel\Vendor\Symfony\Component\HttpClient\Response\StreamableInterface;
use Voxel\Vendor\Symfony\Component\HttpClient\Response\StreamWrapper;
...
final class HttplugWaitLoop
{
     ...
     */
    public function __construct(private HttpClientInterface $client, private ?\SplObjectStorage $promisePool, private ResponseFactoryInterface $responseFactory, private StreamFactoryInterface $streamFactory)
    {
    }
    public function wait(?ResponseInterface $pendingResponse, ?float $maxDuration = null, ?float $idleTimeout = null): int
    {
        ...
        $guzzleQueue = \Voxel\Vendor\GuzzleHttp\Promise\Utils::queue();
        if (0.0 === $remainingDuration = $maxDuration) {
            $idleTimeout = 0.0;
        } elseif (null !== $maxDuration) {
            $startTime = hrtime(\true) / 1000000000.0;
            ...
            foreach ($this->client->stream($this->promisePool, $idleTimeout) as $response => $chunk) {
                try {
                    ...
                    if ([, $promise] = $this->promisePool[$response] ?? null) {
                        ...
                        $promise->resolve(self::createPsr7Response($this->responseFactory, $this->streamFactory, $this->client, $response, \true));
                    }
                } catch (\Exception $e) {
                ...
                check_duration:
                if (null !== $maxDuration && $idleTimeout && $idleTimeout > $remainingDuration = max(0.0, $maxDuration - hrtime(\true) / 1000000000.0 + $startTime)) {
