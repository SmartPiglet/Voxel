...
use Voxel\Vendor\Amp\ByteStream\ReadableBuffer;
use Voxel\Vendor\Amp\ByteStream\ReadableIterableStream;
use Voxel\Vendor\Amp\ByteStream\ReadableResourceStream;
use Voxel\Vendor\Amp\ByteStream\ReadableStream;
use Voxel\Vendor\Amp\Cancellation;
use Voxel\Vendor\Amp\Http\Client\HttpContent;
...
class AmpBodyV5 implements HttpContent, ReadableStream, \IteratorAggregate
{
     ...
     */
    public function __construct($body, &$info, private \Closure $onProgress)
    {
        $this->info =& $info;
        if (\is_resource($body)) {
