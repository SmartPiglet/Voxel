...
final class ArrayShapeType extends CollectionType
{
     ...
     */
    public function __construct(array $shape, private readonly ?Type $extraKeyType = null, private readonly ?Type $extraValueType = null)
    {
        $keyTypes = [];
        $valueTypes = [];
    ...
    public function __toString(): string
    {
        ...
        if (!$this->isSealed()) {
            $items[] = $this->extraKeyType->isIdentifiedBy(TypeIdentifier::INT) && $this->extraKeyType->isIdentifiedBy(TypeIdentifier::STRING) && $this->extraValueType->isIdentifiedBy(TypeIdentifier::MIXED) ? '...' : \sprintf('...<%s, %s>', $this->extraKeyType, $this->extraValueType);
        }
