...
use Voxel\Vendor\Psr\Container\ContainerInterface;
...
final readonly class TypeResolver implements TypeResolverInterface
{
     ...
     */
    public function __construct(private ContainerInterface $resolvers)
    {
    }
    ...
    public static function create(?array $resolvers = null): self
    {
        if (null === $resolvers) {
            ...
            $resolvers = [\ReflectionType::class => $reflectionTypeResolver, \ReflectionParameter::class => new ReflectionParameterTypeResolver($reflectionTypeResolver, $typeContextFactory), \ReflectionProperty::class => new ReflectionPropertyTypeResolver($reflectionTypeResolver, $typeContextFactory), \ReflectionFunctionAbstract::class => new ReflectionReturnTypeResolver($reflectionTypeResolver, $typeContextFactory)];
