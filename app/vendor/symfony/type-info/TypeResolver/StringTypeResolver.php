...
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\ConstExpr\ConstExprArrayNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\ConstExpr\ConstExprFalseNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\ConstExpr\ConstExprFloatNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\ConstExpr\ConstExprIntegerNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\ConstExpr\ConstExprNullNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\ConstExpr\ConstExprStringNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\ConstExpr\ConstExprTrueNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\ConstExpr\ConstFetchNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\ArrayShapeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\ArrayTypeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\CallableTypeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\ConstTypeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\GenericTypeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\IdentifierTypeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\IntersectionTypeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\NullableTypeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\ObjectShapeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\ThisTypeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\TypeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\Type\UnionTypeNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Lexer\Lexer;
use Voxel\Vendor\PHPStan\PhpDocParser\Parser\ConstExprParser;
use Voxel\Vendor\PHPStan\PhpDocParser\Parser\TokenIterator;
use Voxel\Vendor\PHPStan\PhpDocParser\Parser\TypeParser;
use Voxel\Vendor\PHPStan\PhpDocParser\ParserConfig;
use Voxel\Vendor\Symfony\Component\TypeInfo\Exception\InvalidArgumentException;
...
use Voxel\Vendor\Symfony\Component\TypeInfo\Type\BackedEnumType;
...
use Voxel\Vendor\Symfony\Component\TypeInfo\Type\GenericType;
    ...
    private function getTypeFromNode(TypeNode $node, ?TypeContext $typeContext): Type
    {
        ...
        if ($node instanceof ArrayShapeNode) {
            ...
            foreach ($node->items as $item) {
                $shape[(string) $item->keyName] = ['type' => $this->getTypeFromNode($item->valueType, $typeContext), 'optional' => $item->optional];
            }
            return Type::arrayShape($shape, $node->sealed, $node->unsealedType?->keyType ? $this->getTypeFromNode($node->unsealedType->keyType, $typeContext) : null, $node->unsealedType?->valueType ? $this->getTypeFromNode($node->unsealedType->valueType, $typeContext) : null);
        }
        ...
        if ($node instanceof IdentifierTypeNode) {
            $type = match ($node->name) {
                ...
                'string', 'class-string', 'trait-string', 'interface-string', 'callable-string', 'numeric-string', 'lowercase-string', 'non-empty-lowercase-string', 'non-empty-string', 'non-falsy-string', 'truthy-string', 'literal-string', 'html-escaped-string' => Type::string(),
    ...
    private function resolveCustomIdentifier(string $identifier, ?TypeContext $typeContext): Type
    {
        ...
        if (!isset(self::$classExistCache[$className])) {
            ...
            if (class_exists($className) || interface_exists($className)) {
                self::$classExistCache[$className] = \true;
            } else {
                try {
