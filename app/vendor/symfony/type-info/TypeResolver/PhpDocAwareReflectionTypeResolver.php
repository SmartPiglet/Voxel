...
final readonly class PhpDocAwareReflectionTypeResolver implements TypeResolverInterface
{
    ...
    private Lexer $lexer;
    public function __construct(private TypeResolverInterface $reflectionTypeResolver, private TypeResolverInterface $stringTypeResolver, private TypeContextFactory $typeContextFactory, ?PhpDocParser $phpDocParser = null, ?Lexer $lexer = null)
    {
        if (class_exists(ParserConfig::class)) {
            $this->lexer = $lexer ?? new Lexer(new ParserConfig([]));
            $this->phpDocParser = $phpDocParser ?? new PhpDocParser($config = new ParserConfig([]), new TypeParser($config, $constExprParser = new ConstExprParser($config)), $constExprParser);
        } else {
            $this->lexer = $lexer ?? new Lexer();
            $this->phpDocParser = $phpDocParser ?? new PhpDocParser(new TypeParser($constExprParser = new ConstExprParser()), $constExprParser);
        }
    }
    public function resolve(mixed $subject, ?TypeContext $typeContext = null): Type
    {
        ...
        $docComments = match (\true) {
            $subject instanceof \ReflectionProperty => $subject->isPromoted() ? ['@var' => $subject->getDocComment(), '@param' => $subject->getDeclaringClass()?->getConstructor()?->getDocComment()] : ['@var' => $subject->getDocComment()],
