 ...
 */
namespace Voxel\Vendor\Symfony\Component\TypeInfo\TypeContext;

use Voxel\Vendor\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\PhpDoc\TemplateTagValueNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\PhpDoc\TypeAliasImportTagValueNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Ast\PhpDoc\TypeAliasTagValueNode;
use Voxel\Vendor\PHPStan\PhpDocParser\Lexer\Lexer;
use Voxel\Vendor\PHPStan\PhpDocParser\Parser\ConstExprParser;
use Voxel\Vendor\PHPStan\PhpDocParser\Parser\PhpDocParser;
...
use Voxel\Vendor\Symfony\Component\TypeInfo\Exception\LogicException;
use Voxel\Vendor\Symfony\Component\TypeInfo\Exception\RuntimeException;
...
use Voxel\Vendor\Symfony\Component\TypeInfo\TypeResolver\StringTypeResolver;
/**
...
final class TypeContextFactory
{
    ...
    public function __construct(private readonly ?StringTypeResolver $stringTypeResolver = null)
    {
    }
    public function createFromClassName(string $calledClassName, ?string $declaringClassName = null): TypeContext
    {
        ...
        $typeContext = new TypeContext(end($calledClassPath), end($declaringClassPath), trim($declaringClassReflection->getNamespaceName(), '\\'), $this->collectUses($declaringClassReflection));
        return new TypeContext($typeContext->calledClassName, $typeContext->declaringClassName, $typeContext->namespace, $typeContext->uses, $this->collectTemplates($declaringClassReflection, $typeContext), $this->collectTypeAliases($declaringClassReflection, $typeContext));
    }
    public function createFromReflection(\Reflector $reflection): ?TypeContext
    {
        $declaringClassReflection = match (\true) {
        ...
        $typeContext = new TypeContext($declaringClassReflection->getShortName(), $declaringClassReflection->getShortName(), $declaringClassReflection->getNamespaceName(), $this->collectUses($declaringClassReflection));
        $templates = match (\true) {
        ...
        return new TypeContext($typeContext->calledClassName, $typeContext->declaringClassName, $typeContext->namespace, $typeContext->uses, $templates, $this->collectTypeAliases($declaringClassReflection, $typeContext));
    }
    /**
    ...
    private function resolveTypeAliases(array $toResolve, array $resolved, TypeContext $typeContext): array
    {
        ...
        $typeContext = new TypeContext($typeContext->calledClassName, $typeContext->declaringClassName, $typeContext->namespace, $typeContext->uses, $typeContext->templates, $typeContext->typeAliases + $resolved);
        $succeeded = \false;
        $lastFailure = null;
