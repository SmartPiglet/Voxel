 ...
 */
namespace Voxel\Vendor\Symfony\Component\Uid;

use Voxel\Vendor\Symfony\Component\Uid\Exception\InvalidArgumentException;
/**
...
class UuidV6 extends Uuid implements TimeBasedUidInterface
{
    ...
    public function toV7(): UuidV7
    {
        ...
        return new UuidV7(substr_replace(\sprintf('%012s-7%s-%s%s-%s%06s', \PHP_INT_SIZE >= 8 ? dechex($ms) : bin2hex(BinaryUtil::fromBase($ms, BinaryUtil::BASE10)), substr($uuid, -6, 3), $uuid[19], substr($uuid, -3), substr($uuid, -12, 6), $time), '-', 8, 0));
    }
