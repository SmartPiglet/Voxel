 ...
 */
namespace Voxel\Vendor\Symfony\Component\Serializer\Normalizer;

use Voxel\Vendor\Symfony\Component\PropertyAccess\Exception\InvalidArgumentException as PropertyAccessInvalidArgumentException;
use Voxel\Vendor\Symfony\Component\PropertyAccess\Exception\InvalidTypeException;
use Voxel\Vendor\Symfony\Component\PropertyAccess\Exception\NoSuchPropertyException;
use Voxel\Vendor\Symfony\Component\PropertyAccess\Exception\UninitializedPropertyException;
use Voxel\Vendor\Symfony\Component\PropertyAccess\PropertyAccess;
use Voxel\Vendor\Symfony\Component\PropertyInfo\PropertyTypeExtractorInterface;
use Voxel\Vendor\Symfony\Component\PropertyInfo\Type as LegacyType;
use Voxel\Vendor\Symfony\Component\Serializer\Encoder\CsvEncoder;
use Voxel\Vendor\Symfony\Component\Serializer\Encoder\JsonEncoder;
use Voxel\Vendor\Symfony\Component\Serializer\Encoder\XmlEncoder;
use Voxel\Vendor\Symfony\Component\Serializer\Exception\ExtraAttributesException;
use Voxel\Vendor\Symfony\Component\Serializer\Exception\InvalidArgumentException;
use Voxel\Vendor\Symfony\Component\Serializer\Exception\LogicException;
use Voxel\Vendor\Symfony\Component\Serializer\Exception\MissingConstructorArgumentsException;
use Voxel\Vendor\Symfony\Component\Serializer\Exception\NotNormalizableValueException;
use Voxel\Vendor\Symfony\Component\Serializer\Mapping\AttributeMetadata;
use Voxel\Vendor\Symfony\Component\Serializer\Mapping\AttributeMetadataInterface;
use Voxel\Vendor\Symfony\Component\Serializer\Mapping\ClassDiscriminatorFromClassMetadata;
use Voxel\Vendor\Symfony\Component\Serializer\Mapping\ClassDiscriminatorResolverInterface;
use Voxel\Vendor\Symfony\Component\Serializer\Mapping\ClassMetadataInterface;
use Voxel\Vendor\Symfony\Component\Serializer\Mapping\Factory\ClassMetadataFactoryInterface;
use Voxel\Vendor\Symfony\Component\Serializer\NameConverter\NameConverterInterface;
use Voxel\Vendor\Symfony\Component\TypeInfo\Exception\LogicException as TypeInfoLogicException;
use Voxel\Vendor\Symfony\Component\TypeInfo\Type;
use Voxel\Vendor\Symfony\Component\TypeInfo\Type\BuiltinType;
use Voxel\Vendor\Symfony\Component\TypeInfo\Type\CollectionType;
use Voxel\Vendor\Symfony\Component\TypeInfo\Type\IntersectionType;
use Voxel\Vendor\Symfony\Component\TypeInfo\Type\NullableType;
use Voxel\Vendor\Symfony\Component\TypeInfo\Type\ObjectType;
use Voxel\Vendor\Symfony\Component\TypeInfo\Type\UnionType;
use Voxel\Vendor\Symfony\Component\TypeInfo\Type\WrappingTypeInterface;
use Voxel\Vendor\Symfony\Component\TypeInfo\TypeIdentifier;
/**
...
abstract class AbstractObjectNormalizer extends AbstractNormalizer
{
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            try {
                $attributeValue = $attribute === $this->classDiscriminatorResolver?->getMappingForMappedObject($data)?->getTypeProperty() ? $this->classDiscriminatorResolver?->getTypeForMappedObject($data) : $this->getAttributeValue($data, $attribute, $format, $attributeContext);
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        foreach ($normalizedData as $attribute => $value) {
            ...
            if (\false !== $allowedAttributes && !\in_array($attribute, $allowedAttributes, \true) || !$this->isAllowedAttribute($resolvedClass, $attribute, $format, $context)) {
            ...
            try {
                ...
                $exception = NotNormalizableValueException::createForUnexpectedDataType(\sprintf('Failed to denormalize attribute "%s" value for class "%s": ' . $e->getMessage(), $attribute, $resolvedClass), $data, $e instanceof InvalidTypeException ? [$e->expectedType] : ['unknown'], $attributeContext['deserialization_path'] ?? null, \false, $e->getCode(), $e);
                if (isset($context['not_normalizable_value_exceptions'])) {
                    $context['not_normalizable_value_exceptions'][] = $exception;
                    continue;
                }
    ...
    private function validateAndDenormalizeLegacy(array $types, string $currentClass, string $attribute, mixed $data, ?string $format, array $context): mixed
    {
        ...
        foreach ($types as $type) {
            ...
            try {
                ...
                $expectedTypes[LegacyType::BUILTIN_TYPE_OBJECT === $builtinType && $class ? $class : $builtinType] = \true;
                ...
                if (LegacyType::BUILTIN_TYPE_FALSE === $builtinType && \false === $data || LegacyType::BUILTIN_TYPE_TRUE === $builtinType && \true === $data) {
                    return $data;
                }
                switch ($builtinType) {
    ...
    private function validateAndDenormalize(Type $type, string $currentClass, string $attribute, mixed $data, ?string $format, array $context): mixed
    {
        ...
        foreach ($types as $t) {
            ...
            try {
                ...
                $expectedTypes[TypeIdentifier::OBJECT === $typeIdentifier && $class ? $class : $typeIdentifier->value] = \true;
        ...
        } else if ($e && !($type instanceof UnionType && !$type instanceof NullableType)) {
            throw $e;
        }
        if ($context[self::DISABLE_TYPE_ENFORCEMENT] ?? $this->defaultContext[self::DISABLE_TYPE_ENFORCEMENT] ?? \false) {
            return $data;
