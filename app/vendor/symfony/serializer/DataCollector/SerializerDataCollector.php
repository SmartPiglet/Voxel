 ...
 */
namespace Voxel\Vendor\Symfony\Component\Serializer\DataCollector;

use Voxel\Vendor\Symfony\Component\HttpFoundation\Request;
use Voxel\Vendor\Symfony\Component\HttpFoundation\Response;
use Voxel\Vendor\Symfony\Component\HttpKernel\DataCollector\DataCollector;
use Voxel\Vendor\Symfony\Component\HttpKernel\DataCollector\LateDataCollectorInterface;
use Voxel\Vendor\Symfony\Component\Serializer\Debug\TraceableSerializer;
use Voxel\Vendor\Symfony\Component\VarDumper\Cloner\Data;
/**
...
{
    private const DATA_TEMPLATE = ['serialize' => [], 'deserialize' => [], 'normalize' => [], 'denormalize' => [], 'encode' => [], 'decode' => []];
    ...
    public function collectSerialize(string $traceId, mixed $data, string $format, array $context, float $time, array $caller, string $name): void
    {
        ...
        $this->collected[$traceId] = array_merge($this->collected[$traceId] ?? [], compact('data', 'format', 'context', 'time', 'caller', 'name'), ['method' => 'serialize']);
    }
    ...
    {
        unset($context[TraceableSerializer::DEBUG_TRACE_ID]);
        $this->collected[$traceId] = array_merge($this->collected[$traceId] ?? [], compact('data', 'format', 'type', 'context', 'time', 'caller', 'name'), ['method' => 'deserialize']);
    }
    ...
    {
        unset($context[TraceableSerializer::DEBUG_TRACE_ID]);
        $this->collected[$traceId] = array_merge($this->collected[$traceId] ?? [], compact('data', 'format', 'context', 'time', 'caller', 'name'), ['method' => 'normalize']);
    }
    ...
    {
        unset($context[TraceableSerializer::DEBUG_TRACE_ID]);
        $this->collected[$traceId] = array_merge($this->collected[$traceId] ?? [], compact('data', 'format', 'type', 'context', 'time', 'caller', 'name'), ['method' => 'denormalize']);
    }
    ...
    {
        unset($context[TraceableSerializer::DEBUG_TRACE_ID]);
        $this->collected[$traceId] = array_merge($this->collected[$traceId] ?? [], compact('data', 'format', 'context', 'time', 'caller', 'name'), ['method' => 'encode']);
    }
    ...
    {
        unset($context[TraceableSerializer::DEBUG_TRACE_ID]);
        $this->collected[$traceId] = array_merge($this->collected[$traceId] ?? [], compact('data', 'format', 'context', 'time', 'caller', 'name'), ['method' => 'decode']);
    }
    ...
    public function lateCollect(): void
    {
        ...
        foreach ($this->collected as $collected) {
            ...
            $data = ['data' => $this->cloneVar($collected['data']), 'dataType' => get_debug_type($collected['data']), 'type' => $collected['type'] ?? null, 'format' => $collected['format'], 'time' => $collected['time'], 'context' => $this->cloneVar($collected['context']), 'normalization' => [], 'encoding' => [], 'caller' => $collected['caller'] ?? null, 'name' => $collected['name']];
