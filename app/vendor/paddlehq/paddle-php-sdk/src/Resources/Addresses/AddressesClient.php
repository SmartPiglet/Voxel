 ...
 * |-------------------------------------------------------------|.
 */
namespace Voxel\Vendor\Paddle\SDK\Resources\Addresses;

use Voxel\Vendor\Paddle\SDK\Client;
use Voxel\Vendor\Paddle\SDK\Entities\Address;
use Voxel\Vendor\Paddle\SDK\Entities\Collections\AddressCollection;
...
use Voxel\Vendor\Paddle\SDK\Resources\Addresses\Operations\CreateAddress;
use Voxel\Vendor\Paddle\SDK\Resources\Addresses\Operations\ListAddresses;
use Voxel\Vendor\Paddle\SDK\Resources\Addresses\Operations\UpdateAddress;
...
class AddressesClient
{
    ...
    {
        $parser = new ResponseParser($this->client->getRaw("/customers/{$customerId}/addresses", $listOperation));
        return AddressCollection::from($parser->getData(), new Paginator($this->client, $parser->getPagination(), AddressCollection::class));
    }
    /**
     * @throws ApiError          On a generic API error
     * @throws MalformedResponse If the API response was not parsable
    ...
    {
        $parser = new ResponseParser($this->client->getRaw("/customers/{$customerId}/addresses/{$id}"));
    ...
    {
        $parser = new ResponseParser($this->client->postRaw("/customers/{$customerId}/addresses", $createOperation));
        return Address::from($parser->getData());
    }
    /**
     * @throws ApiError                 On a generic API error
    ...
    {
        $parser = new ResponseParser($this->client->patchRaw("/customers/{$customerId}/addresses/{$id}", $operation));
        return Address::from($parser->getData());
    }
    /**
     * @throws ApiError                 On a generic API error
