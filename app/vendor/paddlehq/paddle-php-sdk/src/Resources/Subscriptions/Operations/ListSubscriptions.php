...
use Voxel\Vendor\Paddle\SDK\Entities\Subscription\SubscriptionScheduledChangeAction;
...
class ListSubscriptions implements HasParameters
{
     ...
     */
    public function __construct(private readonly Pager|null $pager = null, private readonly array $addressIds = [], private readonly CollectionMode|null $collectionMode = null, private readonly array $customerIds = [], private readonly array $ids = [], private readonly array $priceIds = [], private readonly array $scheduledChangeActions = [], private readonly array $statuses = [])
    {
    ...
    public function getParameters(): array
    {
        ...
        return array_merge($this->pager?->getParameters() ?? [], array_filter(['address_id' => implode(',', $this->addressIds), 'collection_mode' => $this->collectionMode?->getValue(), 'customer_id' => implode(',', $this->customerIds), 'id' => implode(',', $this->ids), 'price_id' => implode(',', $this->priceIds), 'scheduled_change_action' => implode(',', array_map($enumStringify, $this->scheduledChangeActions)), 'status' => implode(',', array_map($enumStringify, $this->statuses))]));
    }
}
