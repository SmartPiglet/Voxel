...
class ListAdjustments implements HasParameters
{
     ...
     */
    public function __construct(private readonly Pager|null $pager = null, private readonly array $ids = [], private readonly array $statuses = [], private readonly array $customerIds = [], private readonly array $transactionIds = [], private readonly array $subscriptionIds = [], private readonly Action|null $action = null)
    {
    ...
    public function getParameters(): array
    {
        ...
        return array_merge($this->pager?->getParameters() ?? [], array_filter(['id' => implode(',', $this->ids), 'status' => implode(',', array_map($enumStringify, $this->statuses)), 'customer_id' => implode(',', $this->customerIds), 'transaction_id' => implode(',', $this->transactionIds), 'subscription_id' => implode(',', $this->subscriptionIds), 'action' => $this->action?->getValue()]));
    }
}
